...
export function useTagBelongsCreate(){
    ...
    var mutation = useMutation(({seats, tag_name}) => {
        seats = JSON.stringify(seats)
        return new_api.tag_belongs.create(seats, tag_name, map_name, project_name)
    }, {
        onSuccess: ()=>{
            queryClient.invalidateQueries(['tags', {project_name}])
            queryClient.invalidateQueries(['tags_belongs', {map_name, project_name}])
        }
    })
